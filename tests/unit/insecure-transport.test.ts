...
// Imported per test rather than once at the top: the flag is read when the
// module is evaluated, so each case has to set the environment first and then
// load a fresh copy (vi.resetModules drops the cached one).
const load = () => import("../../server/insecure-transport.ts");
