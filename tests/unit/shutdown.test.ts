...
import { GRACE_MS, registerShutdown } from "../../server/shutdown.ts";
...
/* Fire the drain callback the handler gave server.close(). */
const finishDraining = (server: ReturnType<typeof makeFakes>["server"]) => {
  const callback = server.close.mock.calls[0]?.[0];
  if (!callback) throw new Error("server.close was not called");
  callback();
};

type Listener = [string | symbol, (...args: unknown[]) => void];
...
describe("registerShutdown", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(process, "on").mockImplementation((event, fn) => {
      added.push([event, fn as Listener[1]]);
  ...
  it("drains, closes the pool, then exits 0", async () => {
    ...
    finishDraining(server);
    await vi.waitFor(() => expect(exit).toHaveBeenCalledWith(0));
    expect(pool.end).toHaveBeenCalledOnce();
  });

  it("force-exits when draining outlives the grace timeout", () => {
    const { server, pool, exit, log } = makeFakes();
  ...
  it("drains inside Docker's stop_grace_period by default", () => {
