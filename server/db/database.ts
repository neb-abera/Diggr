 ...
 *
 * The Azure SDK is loaded on first use rather than imported: local runs and
 * CI never take this path and need not pay for it.
 */
const entraPassword = () => {
  let credential: import("@azure/identity").DefaultAzureCredential | undefined;
  return async () => {
    if (!credential) {
...
// Re-exported so the rest of the server imports its database types from one
// place rather than from pg directly.
